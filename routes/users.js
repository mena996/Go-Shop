...
router.get('/verify/:id', async (req, res) => {
    ...
    try {
        const user = await UserModel.findByIdAndUpdate(req.params.id, { $set: { verified: 1 } }, { new: true, runValidators: true })
        console.log(user);
        if (!user) next("user not found");
        else res.json("done");
    } catch{
        return res.status(500).send("Internal server error: Can't verify users");
    }
});
